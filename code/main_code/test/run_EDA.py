...
def run_eda(path, lag, season_lag):
    eda = EDA(path)
    df = eda.read_data()
    ts_plt = eda.ts_plots(df)
    acf = eda.acf_plot(df, lag, Col=1)
    acf_pacf = eda.acf_pacf_plot(df, lag, Col=1)
    rolling_mean_var = eda.rolling_mean_var(df, lag, Col=1)
    decomposition = eda.decomposition(df, season_lag, Col=1)
    eda.stationarity(df)
